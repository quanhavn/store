-- Migration: Update import functions to calculate weighted average cost per variant
-- Formula: New Avg Cost = (Old Stock * Old Cost + New Stock * New Cost) / (Old Stock + New Stock)
-- Also updates batch_import_stock to use the same formula

-- Update import_stock_with_variant to use weighted average cost for variants
...
BEGIN
    ...
    IF p_variant_id IS NOT NULL THEN
        -- Get current quantity and cost for weighted average calculation
        SELECT quantity, COALESCE(cost_price, 0)
        ...
        -- New Avg = (Old Stock * Old Cost + New Stock * New Cost) / (Old Stock + New Stock)
        IF p_unit_cost IS NOT NULL AND p_unit_cost > 0 THEN
            IF COALESCE(v_old_qty, 0) + p_quantity > 0 THEN
                v_new_avg_cost := (COALESCE(v_old_qty, 0) * v_old_cost + p_quantity * p_unit_cost) 
                                  / (COALESCE(v_old_qty, 0) + p_quantity);
            ELSE
                v_new_avg_cost := p_unit_cost;
            END IF;
        ELSE
            v_new_avg_cost := v_old_cost;
        END IF;
        
        -- Update variant stock with new weighted average cost
    ...
    ELSE
        -- Get current quantity and cost for weighted average calculation (product level)
        SELECT quantity, COALESCE(cost_price, 0)
        INTO v_old_qty, v_old_cost
        FROM products
        ...
        -- Update product stock with new weighted average cost
        UPDATE products
        SET quantity = quantity + p_quantity,
            cost_price = v_new_avg_cost
...
-- Update batch_import_stock to use weighted average cost for variants
CREATE OR REPLACE FUNCTION batch_import_stock(
    p_store_id UUID,
    p_user_id UUID,
    p_items JSONB,  -- Array of {product_id, variant_id, quantity, unit_cost, item_total}
    p_note TEXT DEFAULT NULL,
    p_record_expense BOOLEAN DEFAULT FALSE,
    p_payment_method VARCHAR DEFAULT 'cash',
    p_bank_account_id UUID DEFAULT NULL,
...
BEGIN
    ...
    LOOP
        ...
        -- Use pre-calculated item_total if provided (avoids rounding issues with unit conversions)
        ...
        IF (v_item->>'item_total') IS NOT NULL AND (v_item->>'item_total')::INTEGER > 0 THEN
            ...
            -- Recalculate unit_cost from item_total for inventory log (may have rounding)
        ...
        IF v_variant_id IS NOT NULL THEN
            ...
            IF v_unit_cost > 0 THEN
                IF COALESCE(v_old_qty, 0) + v_quantity > 0 THEN
                    v_new_avg_cost := (COALESCE(v_old_qty, 0) * v_old_cost + v_quantity * v_unit_cost) 
            ...
            -- Update variant stock with weighted average cost
            UPDATE product_variants
